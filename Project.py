...
def attendance_page():
    ...
    date = st.date_input("Select Date", today, min_value=datetime.date(2024, 1, 1), max_value=today, key="attendance_date")
    date_str = str(date)
...
def todays_attendance_page():
    ...
    if today_data.empty:
        st.info("No attendance data for today.", key="today_info")
    else:
...
def attendance_report():
    ...
    if attendance_df.empty:
        st.warning("No attendance data available.", key="report_warning")
        return
